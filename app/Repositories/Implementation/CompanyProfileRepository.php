...
class CompanyProfileRepository extends BaseRepository implements CompanyProfileRepositoryInterface
{
    ...
    {
        try {
            // Validate & extract extension from base64 header
            if (!preg_match('/^data:image\/(\w+);base64,/', $image_64, $matches)) {
                throw new \Exception("Invalid base64 image format");
            }

            $extension = strtolower($matches[1]); // jpg, png, webp, etc.
            $imageData = substr($image_64, strpos($image_64, ',') + 1);
            $imageData = str_replace(' ', '+', $imageData);

            // Build destination path without hardcoding "/"
            ...
            $filePath = $destinationPath . DIRECTORY_SEPARATOR . $imageName;

            // Save file safely
            $saved = file_put_contents($filePath, base64_decode($imageData));
            if ($saved === false) {
                return '';
            }
